...
export function Sidebar({ totalQuestions, currentIndex, answers, bookmarked, questionIds, onNavigate, onFinish, durationSeconds = 90 * 60 }: SidebarProps) {
    ...
    return (
        <>
            <div className="w-72 flex-shrink-0 flex flex-col bg-[#0b0c10] border-l border-white/5 h-[calc(100vh-64px)] hidden lg:flex relative">

                {/* Top: Timer Component */}
                <div className="p-6 border-b border-white/5 bg-[#0b0c10] z-10">
                ...
                {/* Headers */}
                <div className="px-6 py-4 bg-[#0b0c10] border-b border-white/5">
                    <h3 className="text-sm font-medium text-slate-400">Total Questions: {totalQuestions}</h3>
                </div>
                        ...
                        {Array.from({ length: totalQuestions }).map((_, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        "flex items-center justify-between px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] transition-colors text-left",
                                        isCurrent && "bg-blue-900/10 border-l-2 border-l-blue-500" // Highlight current
                                    )}
                                >
                                        ...
                                        <div className="shrink-0">
                                            {isAnswered ? (
                                                ...
                                                <div className={cn(
                                                    ...
                                                    isCurrent ? "border-blue-500" : "border-slate-600"
                                                )} />
                                            )}
                                        </div>
                ...
                <div className="px-6 py-4 border-t border-white/5 bg-[#0b0c10] sticky bottom-0 z-10 w-full mt-auto">
                    <Button
                        onClick={handleFinishClick}
                        variant="outline"
                        className="w-full border-2 border-blue-600/50 text-blue-500 hover:bg-blue-600 hover:text-white font-bold h-12 rounded-lg transition-all"
                    >
                        SUBMIT TEST
                    </Button>
                </div>
            ...
            {showConfirmModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-[#15171e] text-white p-6 rounded-xl border border-white/10 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <h3 className="text-lg font-bold mb-2 text-white">Finish Exam?</h3>
